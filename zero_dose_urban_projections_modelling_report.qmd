...
# Add measles-specific metrics for inline text references
# Measles coverage gap in 2025
global_measles_gap_2025 <- coverage_summary_combined %>% 
  filter(country == "Global", year == 2025, vaccine == "measles1", method == "historical_progress") %>% 
  summarize(diff = urban_nonpoor_coverage - urban_poor_coverage) %>% 
  pull(diff)

# Measles coverage for urban strata
measles_urban_nonpoor_2025 <- coverage_summary_combined %>% 
  filter(country == "Global", year == 2025, vaccine == "measles1", method == "historical_progress") %>% 
  select(urban_nonpoor_coverage) %>% 
  pull()

measles_urban_poor_2025 <- coverage_summary_combined %>% 
  filter(country == "Global", year == 2025, vaccine == "measles1", method == "historical_progress") %>% 
  select(urban_poor_coverage) %>% 
  pull()

measles_2050_historical <- coverage_summary_combined %>% 
  filter(country == "Global", year == 2050, vaccine == "measles1", method == "historical_progress") %>% 
  select(overall_coverage) %>% 
  pull()

measles_urban_nonpoor_2050 <- coverage_summary_combined %>% 
  filter(country == "Global", year == 2050, vaccine == "measles1", method == "historical_progress") %>% 
  select(urban_nonpoor_coverage) %>% 
  pull()

measles_urban_poor_2050 <- coverage_summary_combined %>% 
  filter(country == "Global", year == 2050, vaccine == "measles1", method == "historical_progress") %>% 
  select(urban_poor_coverage) %>% 
  pull()

# Measles zero-dose metrics
measles_zero_dose_static_2050 <- zero_dose_summary_combined %>% 
  filter(country == "Global", year == 2050, vaccine == "measles1", method == "static") %>% 
  pull(urban_poor_zero_dose)

measles_zero_dose_historical_2050 <- zero_dose_summary_combined %>% 
  filter(country == "Global", year == 2050, vaccine == "measles1", method == "historical_progress") %>% 
  pull(urban_poor_zero_dose)

measles_zero_dose_static_2025 <- zero_dose_summary_combined %>% 
  filter(country == "Global", year == 2025, vaccine == "measles1", method == "static") %>% 
  pull(urban_poor_zero_dose)

measles_zero_dose_diff_2050 <- measles_zero_dose_static_2050 - measles_zero_dose_historical_2050

# Create measles summary tables for coverage
measles_summary_table <- coverage_summary_combined %>%
  filter(year %in% c(2025, 2030, 2050)) %>%
  filter(country %in% country_order) %>%
  filter(vaccine == "measles1") %>%
  select(country, year, method, overall_coverage) %>%
  pivot_wider(
    names_from = c(year, method),
    values_from = overall_coverage,
...
# Create measles zero-dose summary table
measles_zero_dose_table <- zero_dose_summary_combined %>%
  filter(year %in% c(2025, 2030, 2050)) %>%
  filter(country %in% country_order) %>%
  filter(vaccine == "measles1") %>%
  select(country, year, method, total_zero_dose) %>%
...
Our analysis reveals substantial disparities in vaccination coverage between urban poor and urban non-poor populations across all countries studied. As of 2025, the global average DTP1 coverage was `r format_pct(urban_nonpoor_2025)` for urban non-poor populations compared to `r format_pct(urban_poor_2025)` for urban poor populations, representing a gap of `r format_pct(global_dtp1_gap_2025)` percentage points. For measles vaccination, we observed similar disparities with `r format_pct(measles_urban_nonpoor_2025)` coverage in urban non-poor populations compared to `r format_pct(measles_urban_poor_2025)` in urban poor populations, a gap of `r format_pct(global_measles_gap_2025)` percentage points.
...
Using beta regression models fitted to historical data (2000-2019), we projected vaccination coverage for both DTP1 and measles vaccines for all countries through 2050. Under the trend-based scenario, global DTP1 coverage is expected to reach `r format_pct(dtp1_2050_historical)` by 2050, representing a `r format_pct(dtp1_2050_historical - mean(c(urban_nonpoor_2025, urban_poor_2025)))` percentage point increase from the 2025 baseline. For measles vaccination, global coverage is projected to reach `r format_pct(measles_2050_historical)` by 2050 under the trend-based scenario.

However, these aggregate figures mask important differences between population strata. By 2050, DTP1 coverage is projected to reach `r format_pct(urban_nonpoor_2050)` for urban non-poor populations but only `r format_pct(urban_poor_2050)` for urban poor populations. Similarly, measles coverage is expected to reach `r format_pct(measles_urban_nonpoor_2050)` for urban non-poor populations compared to `r format_pct(measles_urban_poor_2050)` for urban poor populations, indicating persistent disparities throughout the projection period.

The timing differences between the two vaccines, with measles typically administered at 1 year of age whereas DTP doses are given earlier in the first year of life, result in distinct coverage patterns. These differences reflect both the sequential nature of vaccination schedules and the specific challenges associated with returning for follow-up vaccinations, particularly for urban poor populations with higher mobility and opportunity costs.

@tbl-coverage-projections summarises the projected DTP1 coverage rates for selected countries across both projection methods, demonstrating the divergence between trend-based and static scenarios over time.

```{r}
#| label: tbl-coverage-projections
#| tbl-cap: "DTP1 vaccination coverage (%) under historical progress and static methods (2025-2050). Values represent projected vaccination coverage across urban population strata."
...
```{r}
#| label: tbl-measles-coverage-projections
#| tbl-cap: "Measles vaccination coverage (%) under historical progress and static methods (2025-2050). Values represent projected measles vaccination coverage across urban population strata."

# Format the measles summary table for display
measles_coverage_table <- measles_summary_table %>%
  rename(Country = country) %>%
  mutate(across(starts_with("coverage_"), ~paste0(round(., 1), "%")))
...
# Check if all expected columns are present
missing_cols <- setdiff(col_names, names(measles_coverage_table))
if (length(missing_cols) > 0) {
  warning("Missing columns: ", paste(missing_cols, collapse = ", "))
  # Add missing columns with NA values
  for (col in missing_cols) {
    measles_coverage_table[[col]] <- NA
  }
  # Ensure columns are in correct order
  measles_coverage_table <- measles_coverage_table %>% select(all_of(col_names))
}

# Simplified table output
knitr::kable(measles_coverage_table, 
             booktabs = TRUE,
             col.names = c("Country", "2025 Historical Progress", "2025 Static", "2030 Historical Progress", "2030 Static", 
                          "2050 Historical Progress", "2050 Static"),
             align = c("l", rep("c", 6))) %>%
...
@fig-dtp-main and @fig-measles-main provide a visual comparison of the two projection methods for DTP1 and measles coverage across selected countries. The trend-based projections show continued improvement in coverage aligned with historical patterns, while static projections maintain 2025 levels through 2050. The divergence between methods increases over time, with substantial differences by 2050. @tbl-measles-coverage-projections shows similar projection patterns for measles vaccination, though with different baseline values and trajectories reflecting the distinct challenges of measles vaccination.

## Zero-Dose Children Projections

The contrasting coverage trajectories between the two projection methods translate into significant differences in the projected number of zero-dose children for both DTP1 and measles vaccines. Under static coverage projections, the number of zero-dose children in urban poor settings globally would increase from `r format_num(zero_dose_summary_combined %>% filter(country == "Global", year == 2025, vaccine == "DTP1", method == "static") %>% pull(urban_poor_zero_dose))` in 2025 to `r format_num(global_zero_dose_static_2050)` by 2050 for DTP1, despite maintaining constant coverage rates.

For measles vaccination, the number of zero-dose children in urban poor settings would similarly increase from `r format_num(measles_zero_dose_static_2025)` in 2025 to `r format_num(measles_zero_dose_static_2050)` by 2050 under static coverage projections. This paradoxical increase occurs because population growth in urban areas outpaces the effect of constant vaccination coverage.

In contrast, with historical progress projections that assume continued historical improvements in coverage, the number of DTP1 zero-dose children in urban poor settings would decrease to `r format_num(global_zero_dose_historical_2050)` by 2050, while measles zero-dose children would decrease to `r format_num(measles_zero_dose_historical_2050)`. This represents a difference of `r format_num(global_zero_dose_diff_2050)` and `r format_num(measles_zero_dose_diff_2050)` children for DTP1 and measles respectively who would remain unvaccinated under static coverage compared to trend-based projections.

These differences reflect both the shared systemic barriers affecting the entire immunization schedule and the specific challenges associated with measles vaccination, which typically occurs at 1 year of age, well after the DTP doses given in the first months of life.
...
```{r}
#| label: tbl-vaccine-comparison-2050
#| tbl-cap: "Comparison of DTP1 and measles zero-dose children in urban poor settings by 2050 (thousands). Difference represents additional zero-dose children under static coverage compared to historical progress projections."

# Create comparison table between DTP1 and measles for 2050
vaccine_comparison <- bind_rows(
  zero_dose_summary_combined %>%
    filter(country %in% country_order, year == 2050, vaccine == "DTP1") %>%
    select(country, method, urban_poor_zero_dose) %>%
    mutate(vaccine = "DTP1"),
  
  zero_dose_summary_combined %>%
    filter(country %in% country_order, year == 2050, vaccine == "measles1") %>%
    select(country, method, urban_poor_zero_dose) %>%
    mutate(vaccine = "Measles")
) %>%
  mutate(country = factor(country, levels = country_order)) %>%
  arrange(country, vaccine, method) %>%
  pivot_wider(
    id_cols = c(country, vaccine),
    names_from = method,
    values_from = urban_poor_zero_dose
  ) %>%
  mutate(
    # Calculate difference between methods
    difference = static - historical_progress,
    # Format values as thousands with 1 decimal place
    historical_progress = round(historical_progress/1000, 1),
    static = round(static/1000, 1),
    difference = round(difference/1000, 1)
  )

# Create the formatted table
knitr::kable(vaccine_comparison, 
             booktabs = TRUE,
             col.names = c("Country", "Vaccine", "Historical Progress", "Static", "Difference"),
             align = c("l", "l", "r", "r", "r")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                full_width = FALSE,
                font_size = 8) %>%
  add_header_above(c(" " = 2, "Zero-dose children in 2050 (thousands)" = 3))
```

@tbl-vaccine-comparison-2050 provides a direct comparison between DTP1 and measles zero-dose projections for 2050, illustrating both the differences in outcomes between the two vaccines and the substantial impact of the projection method on the number of unvaccinated children. This comparison highlights that while both vaccines face similar challenges, the specific issues of measles vaccination result in distinct patterns of zero-dose children across countries.
...
```{r}
#| label: fig-dtp-main
#| fig-cap: "DTP1 vaccination coverage and zero-dose children by urban population strata. The left panels show global projections while right panels show country-specific patterns. Upper panels display coverage percentages while lower panels show absolute numbers of DTP1 zero-dose children (in thousands). Urban poor populations consistently show lower DTP1 coverage and higher numbers of zero-dose children despite proximity to health services."
#| out-width: "100%"
#| fig-width: 16
#| fig-height: 10
...
The delay between DTP1 and measles vaccination in most immunization schedules (with DTP doses administered at 6, 10, and 14 weeks and measles vaccination at 1 year of age) creates additional opportunities for children to miss vaccinations, particularly in highly mobile urban poor populations. This timing difference contributes to distinct patterns in coverage and zero-dose numbers between the two vaccines, with some countries showing greater disparities for measles than DTP1, reflecting compounding barriers at different points in the immunization schedule.
...
@tbl-zero-dose below provides a detailed country-level breakdown of projected zero-dose children for 2023 and 2050 across the different scenarios, highlighting the magnitude of the challenge in specific high-burden countries.

```{r}
#| label: tbl-zero-dose
#| tbl-cap: "Zero-dose children for DTP1 by country and urban strata (thousands, 2025 vs 2050)"

# Create table of zero-dose children - focus only on urban strata
zero_dose_table_data <- zero_dose_summary_combined %>%
...
dtp1_zero_dose <- zero_dose_table_data %>%
...
colnames(dtp1_zero_dose) <- gsub("_zero_dose_", " ", colnames(dtp1_zero_dose))
colnames(dtp1_zero_dose) <- gsub("_", " ", colnames(dtp1_zero_dose))

# Select columns for the standard table
...
missing_cols <- setdiff(select_cols, names(dtp1_zero_dose))
if (length(missing_cols) > 0) {
  for (col in missing_cols) {
    dtp1_zero_dose[[col]] <- NA
  }
}

# Create the standard table
knitr::kable(
  dtp1_zero_dose[, select_cols],
  caption = "DTP1 Urban Poor Zero-Dose Children (thousands)",
...
```{r}
#| label: tbl-measles-zero-dose
#| tbl-cap: "Zero-dose children for measles vaccination by country and urban strata (thousands, 2025 vs 2050)"

# Create table of zero-dose children for measles - focus only on urban strata
measles_zero_dose_data <- zero_dose_summary_combined %>%
  filter(year %in% c(2025, 2050)) %>%
  filter(vaccine == "measles1") %>%
  filter(country %in% country_order) %>%  # Use consistent country order
  mutate(country = factor(country, levels = country_order)) %>%  # Ensure Global is first
...
# Create a safer pivot with complete combinations
measles_zero_dose <- measles_zero_dose_data %>%
  complete(country, year_method) %>%
  pivot_wider(
    id_cols = country,
    names_from = year_method,
...
# Format column names for display
colnames(measles_zero_dose) <- gsub("_zero_dose_", " ", colnames(measles_zero_dose))
colnames(measles_zero_dose) <- gsub("_", " ", colnames(measles_zero_dose))
...
# Handle missing columns
missing_cols <- setdiff(select_cols, names(measles_zero_dose))
if (length(missing_cols) > 0) {
  for (col in missing_cols) {
    measles_zero_dose[[col]] <- NA
  }
}

# Create the standard table
knitr::kable(
  measles_zero_dose[, select_cols],
  caption = "Measles Urban Poor Zero-Dose Children (thousands)",
  col.names = c("Country", "2025 Historical Progress", "2050 Historical Progress", "2025 Static", "2050 Static"),
  booktabs = TRUE
) %>%
  kable_styling(font_size = 9, 
...
The tables above provide detailed country-level breakdowns of projected zero-dose children for both DTP1 and measles vaccines for 2025 and 2050 across the different scenarios. These tables highlight the magnitude of the challenge in specific high-burden countries and demonstrate how measles vaccination faces both shared and distinct barriers compared to DTP1 across countries.
...
The projected increase in zero-dose children under static coverage—from `r format_num(zero_dose_summary_combined %>% filter(country == "Global", year == 2025, vaccine == "DTP1", method == "static") %>% pull(urban_poor_zero_dose))` in 2025 to `r format_num(global_zero_dose_static_2050)` by 2050 for DTP1 and from `r format_num(measles_zero_dose_static_2025)` to `r format_num(measles_zero_dose_static_2050)` for measles in urban poor settings globally—illustrates this paradox. Despite maintaining percentage coverage at 2025 levels, the absolute number of unvaccinated children increases by `r format_pct((global_zero_dose_static_2050 / (zero_dose_summary_combined %>% filter(country == "Global", year == 2025, vaccine == "DTP1", method == "static") %>% pull(urban_poor_zero_dose)) - 1) * 100)` for DTP1 and `r format_pct((measles_zero_dose_static_2050 / measles_zero_dose_static_2025 - 1) * 100)` for measles over the projection period. This increase occurs because the denominator—the eligible population—grows faster than the numerator—the number of vaccinated children—when coverage rates remain static.
...
Our analysis reinforces previous findings on health disparities within urban settings [@belt2023; @santos2024], but extends this work by quantifying the potential future trajectory of these disparities. The `r format_pct(global_dtp1_gap_2025)` percentage point gap in coverage between urban poor and urban non-poor populations at baseline is likely to persist even as overall coverage improves. We have therefore maintained this gap as fixed in our projections, though targeted interventions could potentially reduce this disparity over time.
...
5.  **Vaccine-specific approaches may be necessary**: Our analysis shows that while patterns are similar for DTP and measles vaccines, there are differences in baseline coverage levels and trajectories that may reflect distinct challenges in delivery timing, perception of disease risk, or historical campaign approaches. For instance, measles coverage in urban poor settings starts at `r format_pct(measles_urban_poor_2025)` compared to `r format_pct(urban_poor_2025)` for DTP1, and by 2050, these are projected to reach `r format_pct(measles_urban_poor_2050)` and `r format_pct(urban_poor_2050)` respectively. These differences suggest that while system-strengthening approaches are essential, vaccine-specific strategies may also be necessary.

The sequential nature of childhood immunization schedules creates particular challenges for measles vaccination, which typically occurs at 1 year of age, well after the DTP doses given in the first months of life. Our projections show that the number of measles zero-dose children in urban poor settings would increase from `r format_num(measles_zero_dose_static_2025)` in 2025 to `r format_num(measles_zero_dose_static_2050)` by 2050 under static coverage scenarios. Countries with high mobility among urban poor populations may need targeted measles strategies beyond those used for DTP, such as periodic intensification of routine immunization activities or integration with other health services frequently accessed by these populations. The country-specific variations in the DTP1-measles gap evident in @tbl-vaccine-comparison-2050 further support the need for tailored approaches rather than universal solutions.
...
Effective interventions to reduce zero-dose prevalence in urban areas will require multifaceted approaches that address both supply and demand-side barriers. Based on the literature and our findings, these should include engaging community health workers to identify and track zero-dose children, which has shown success in urban settings like Dar es Salaam [@tanzania2023]; linking immunization with antenatal care and childbirth services to reduce missed opportunities for vaccination in urban poor populations [@hogan2023]; leveraging technology for microplanning and real-time monitoring to improve service delivery in dense urban environments [@ingle2023]; addressing gender-related barriers, such as limited decision-making power among women, which is essential for improving immunization rates in urban poor settings [@ducharme2023]; and tailoring health messaging and services to the needs of urban poor, migrants, and marginalized groups to enhance trust and uptake of immunization services [@tiwari2024].
...
The data and code used for this analysis are available in a public repository at [https://github.com/bquilty25/zdup](https://github.com/bquilty25/zdup). This includes the raw data files, R implementation code, and analysis scripts to reproduce all figures and tables. The population projection data were sourced from UN World Population Prospects [@worldpopulationprospects], with urban proportions from the World Urbanization Prospects [@worldurbanizationprospects]. Historical vaccination coverage data were obtained from WHO/UNICEF estimates [@whounicefestimates], all of which are publicly available from their respective sources. Urban poor and non-poor coverage data were sourced from a detailed analysis of Demographic and Health Surveys [@santos2024].
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.width=16, fig.height=20}
